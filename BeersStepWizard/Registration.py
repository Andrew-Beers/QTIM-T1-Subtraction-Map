...
class RegistrationStep( BeersSingleStep ) :
	...
	def __init__( self, stepid ):
		...
		self.setDescription( """Please select your preferred method of registration. If you have already registered your images, 
			or no registration is required, check the option "No Registration." Be aware that many other modules in Slicer have 
			more complex and/or customizable registration methods, should the methods in this step prove insufficient.
			""")
	...
	def createUserInterface( self ):
		...
		elf.__registrationButton = qt.QPushButton('Run registration')
		...
		self.radio2.toolTip = """Computes a rigid registration on the pre-contrast image with respect to the post-contrast image. 
		This is likely to be the fastest registration method"""
		...
		self.RegistrationRadio.layout().addWidget(self.radio3)
		
		self.radio4 = qt.QRadioButton("BSpline Registration (Slowest)")
		self.radio4.toolTip = """Computes a BSpline Registration on the pre-contrast image with respect to the post-contrast image. This 
		method is slowest and may be necessary for only severly distorted images."""
		self.RegistrationRadio.layout().addWidget(self.radio4)
  ...
  def validate(self, desiredBranchId):
	...
	validationStatus = self.__registrationStatus.getText()

	if validationStatus == 'Register Scans' or validationStatus == 'Registration Completed':
